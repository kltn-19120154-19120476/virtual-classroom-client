...
import Heading from "src/components/Presentation/Heading";
import MultipleChoice from "src/components/Presentation/MultipleChoice";
import Paragraph from "src/components/Presentation/Paragraph";
    ...
    const handleRemovePresentation = async (id) => {
        try {
            ...
            if (res?.status === "OK") {
                await customToast(
                    "SUCCESS",
                    `Delete presentation ${presentation?.name} successfully!`,
                );
            } else {
        ...
        window.location.href = getLinkWithPrefix("/presentation");
    };
    ...
    const handleAssignGroup = async (e) => {
        try {
            ...
            const otherPresentations =
                user?.myPresentations?.filter(
                    (p) => p._id !== presentation._id && p.groupId === groupId,
                ) || [];
            await Promise.all(
                otherPresentations?.map((p) =>
                    updatePresentation({ ...p, groupId: "", isPresent: false }),
                ),
            );
            ...
            if (res?.status === "OK") {
                ...
                if (groupId) {
                    await customToast(
                        "SUCCESS",
                        `Assign group to ${presentation.name} successfully!`,
                    );
                } else {
                    await customToast(
                        "SUCCESS",
                        `Presentation ${presentation.name} is public now`,
                    );
                }
            } else {
                await customToast("ERROR", res?.message);
        ...
        <>
            ...
            >
                <DialogTitle id="alert-dialog-title">
                    Please confirm to delete this presentation
                </DialogTitle>
                <DialogActions>
                    <Button
            ...
            {slides ? (
                        ...
                        <Grid container item xs={12}>
                            ...
                            >
                                {user?._id === presentation?.ownerId && (
                                    <Link
                                        href={`/presentation/${id}/collaboration`}
                                    >
                                        <Button
                                            className="custom-button-outlined"
                ...
                <p style={{ textAlign: "center" }}>
                    This pesentation cannot be found
                </p>
            )}
        </>
    );
