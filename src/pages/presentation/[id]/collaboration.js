...
export default function GroupDetailPage() {
    ...
    const [openInviteCollaboratorForm, setOpenInviteCollaboratorForm] =
        useState(false);
    ...
    const getInfoOfPresentation = async () => {
        try {
            ...
            if (res.status === "OK") {
                ...
                userListRes?.data?.forEach(
                    (user) => (userListMap[user?._id] = user),
                );

                presentationInfo.collaborators =
                    presentationInfo.collaborators.map(
                        (code) => userListMap[code],
                    );
    ...
    const handleRemove = async (collaborator) => {
        try {
            ...
            if (res?.status === "OK") {
                await customToast(
                    "SUCCESS",
                    `Remove collaborator ${collaborator.name} successfully!`,
                );
                router.reload();
            } else {
                await customToast("ERROR", e.response?.data?.message);
    ...
    const handleDeletePresentation = async () => {
        try {
            ...
            if (res?.status === "OK") {
                await customToast(
                    "SUCCESS",
                    `Remove presentation ${presentation.name} successfully!`,
                );
                ...
                <TableContainer component={Paper}>
                    <Table sx={{ minWidth: 650 }} aria-label="simple table">
                        ...
                        <TableBody>
                            ...
                            {presentation?.collaborators?.map(
                                (collaborator) => (
                                    <TableRow
