...
const UserDatailsComponent = (props) => {
  const { isOpen, user, dataJson } = props;
  ...
  const { createdOn, endedOn, polls, users } = dataJson;
  ...
  const userEndOffsetTime = isOnline ? 0 : (((endedOn || currTime()) - leftTime) * 100) / sessionDuration;
  ...
  const talkTimeAverage = usersTalkTime.reduce((prev, curr) => prev + curr, 0) / (allUsersArr.length || 1);

  const messagesAverage = usersMessages.reduce((prev, curr) => prev + curr, 0) / (allUsersArr.length || 1);

  const emojisAverage = usersEmojis.reduce((prev, curr) => prev + curr, 0) / (allUsersArr.length || 1);

  const raiseHandsAverage = usersRaiseHands.reduce((prev, curr) => prev + curr, 0) / (allUsersArr.length || 1);

  const pollsAverage = usersAnswers.reduce((prev, curr) => prev + curr, 0) / (allUsersArr.length || 1);
  ...
  function renderPollItem(poll, answers) {
    ...
    const answersSorted = Object.entries(pollVotesCount[pollId]).sort(([, countA], [, countB]) => countB - countA);
    ...
    const capitalizeFirstLetter = (text) => String.fromCharCode(text.charCodeAt(0) - 32) + text.substring(1);

    return (
      <tr className="p-6 flex flex-row justify-between items-center">
        <td className="min-w-[40%] text-ellipsis">{question}</td>
        {isAnonymous ? (
          <td className="min-w-[20%] grow text-center mx-3">
          ...
          <td className="min-w-[20%] grow text-center mx-3">
            {answers.map((answer) => (
              <p title={answer} key={answer} className="overflow-hidden text-ellipsis">
                {answer}
              </p>
            ))}
          </td>
        ...
        <td
          ...
          title={mostCommonAnswer ? capitalizeFirstLetter(mostCommonAnswer) : null}
        >
          {mostCommonAnswer ? capitalizeFirstLetter(mostCommonAnswer) : "Not available"}
        </td>
      </tr>
    );
  }

  function renderActivityScoreItem(category, average, activityPoints, totalOfActivity) {
    return (
      <tr className="p-6 flex flex-row justify-between items-end">
        <td className="min-w-[20%] text-ellipsis overflow-hidden">category</td>
        <td className="min-w-[60%] grow text-center text-sm">
          <div className="mb-2">
            {(function getAverage() {
              if (average >= 0 && category === "Talk Time") return tsToHHmmss(average);
              if (average >= 0 && category !== "Talk Time") return average;
        ...
        <td className="min-w-[20%] text-sm text-ellipsis overflow-hidden text-right rtl:text-left">
          {activityPoints >= 0 ? activityPoints : "Not available"}
        </td>
      </tr>
    );
  }
  ...
  function getUserAnswer(poll) {
    if (typeof user.answers[poll.pollId] !== "undefined") {
      return Array.isArray(user.answers[poll.pollId]) ? user.answers[poll.pollId] : [user.answers[poll.pollId]];
    }
    return [];
  }

  const Duration = new Date(getSumOfTime(Object.values(user.intIds))).toISOString().substring(11, 19);

  return (
    <div className="fixed inset-0 flex flex-row z-50">
      <div className="bg-black grow opacity-50" role="none" onClick={() => console.log("closeModal")} />
      <div ref={modalRef} className="overflow-auto w-full md:w-2/4 bg-gray-100 p-6">
        ...
        <div className="bg-white shadow rounded mb-4">
          <div className="p-6 text-lg flex items-center">
            ...
            <h3 className="ltr:ml-2 rtl:mr-2">Overview</h3>
          </div>
          <div className="p-6 m-px bg-gray-100">
            <div className="h-6 relative before:bg-gray-500 before:absolute before:w-[10px] before:h-[10px] before:rounded-full before:left-0 before:top-[calc(50%-5px)] after:bg-gray-500 after:absolute after:w-[10px] after:h-[10px] after:rounded-full after:right-0 after:top-[calc(50%-5px)]">
              ...
              <div
                role="progressbar"
                aria-label={`${user.name} online time ${Duration}`}
              ...
              >
                <div className="mx-3 inline-block text-white">{Duration}</div>
              </div>
            </div>
            <div className="flex flex-row justify-between font-light text-gray-700">
              <div aria-label={`Joined ${new Date(createdOn).toISOString().substring(11, 19)}`}>
                <div aria-hidden="true">Start Time</div>
                <div aria-hidden="true">{formatTime(createdOn)}</div>
              </div>
              <div className="ltr:text-right rtl:text-left">
                <div aria-hidden="true">End Time</div>
                <div>
                  {endedOn === 0 ? (
          ...
          <div className="p-6 flex flex-row justify-between text-gray-700">
            <div aria-label={`Duration ${Duration}`}>
              ...
              <div aria-hidden="true">Duration</div>
            </div>
            <div aria-label={`Joined ${new Date(joinTime).toISOString().substring(11, 19)}`}>
              <div aria-hidden="true" className="font-medium">
                {formatTime(joinTime)}
              </div>
              <div aria-hidden="true">Joined</div>
            </div>
            <div>
          ...
          <>
            <div className="bg-white shadow rounded mb-4 w-full">
              <div className="p-6 text-lg flex items-center">
                ...
                <h3 className="ltr:ml-2 rtl:mr-2">
                  Activity Score :&nbsp;
                  <span className="font-bold">{getActivityScore(user, users, totalPolls)}</span>
                </h3>
              </div>
              <table className="bg-white shadow rounded mb-4 table w-full">
                <tr className="p-6 py-2 m-px bg-gray-200 flex flex-row justify-between text-xs text-gray-700">
                  <th aria-label="Category" className="min-w-[20%] text-ellipsis font-normal text-left">
                    Category
                  </th>
                  <th aria-label="Average" className="grow text-center font-normal">
                    Average
                  </th>
                  <th aria-label="Activity Points" className="min-w-[20%] text-ellipsis text-right rtl:text-left font-normal">
                    Activity Points
                  </th>
                </tr>
                {["Talk Time", "Messages", "Emojis", "Raise Hands", "Poll Votes"].map((category) => {
                  ...
                  return renderActivityScoreItem(category, averages[category], activityPointsFunctions[category](user), totalOfActivity);
                })}
              </table>
            </div>
            <div className="bg-white shadow rounded">
              <div className="p-6 text-lg flex items-center">
                ...
                <h3 className="ltr:ml-2 rtl:mr-2">Polls</h3>
              </div>
              <table className="w-full">
                <tr className="p-6 py-2 m-px bg-gray-200 flex flex-row justify-between text-xs text-gray-700">
                  <th aria-label="Poll" className="min-w-[40%] text-ellipsis font-normal text-left">
                    Poll
                  </th>
                  <th aria-label="Response" className="grow text-center font-normal">
                    Response
                  </th>
                  <th aria-label="Most Common Answer" className="min-w-[40%] text-ellipsis text-center font-normal">
                    Most Common Answer
                  </th>
                </tr>
                {Object.values(polls || {}).map((poll) => renderPollItem(poll, getUserAnswer(poll)))}
              </table>
            </div>
          </>
        )}
...
export default UserDatailsComponent;
