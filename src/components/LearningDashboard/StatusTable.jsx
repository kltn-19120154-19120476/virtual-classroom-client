...
class StatusTable extends React.Component {
  ...
  render() {
    ...
    function makeLineThrough(userPeriod, period) {
      ...
      return (
        <div
          className={"h-1.5 bg-gray-200 absolute inset-x-0 z-10" + ` ${width} ${roundedLeft} ${roundedRight}`}
          style={{
            top: `calc(50% - ${redress})`,
      ...
      <table className="w-full">
        <thead>
          <tr className="text-xs font-semibold tracking-wide text-gray-700 uppercase border-b bg-gray-100">
            <th className={`z-30 bg-inherit px-4 py-3 col-text-left sticky ${isRTL ? "right-0" : "left-0"}`}>User</th>
            <th className="bg-inherit" colSpan={periods.length}>
              <span className="invisible" aria-label={"Timeline"}>
                Timeline
              </span>
            </th>
          </tr>
                      ...
                      <div className="flex">
                        <div className="my-4">
                          ...
                          <p id={`thumb-desc-${presentationId}`} className="absolute w-0 h-0 p-0 border-0 m-0 overflow-hidden">
                            {`Presentation thumbnail - presentation,
                            )} ${presentationName} - page number ${pageNum} - set at ${start}`}
                          </p>
                ...
                .map((user) => (
                  <tr className="text-gray-700 bg-inherit" key={user.userKey}>
                    ...
                    {periods.map((period) => {
                      ...
                      const interval = period.end - period.start;
                      return (
                        <td className="relative px-3.5 2xl:px-4 py-3 text-sm col-text-left" key={`${user.userKey}-period`}>
                                  ...
                                  <>
                                    ...
                                    {userEmojisInPeriod.map((emoji) => {
                                      ...
                                      return (
                                        ...
                                        >
                                          <i className={"text-sm bbb-icon-timeline" + ` ${emojiConfigs[emoji.name].icon}`} />
                                        </div>
                                      );
                                    })}
                                  </>
