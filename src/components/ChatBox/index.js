...
import { clearChat, getChatPaging, saveChat } from "src/client/presentation";
...
export default function ChatBox({ room, owner }) {
  ...
  const { isLoading, isError, error, results, hasNextPage } = useMessageLoading(
    pageNumber,
    room,
  );
  ...
  const lastMessRef = useCallback(
    (node) => {
      ...
      observer.current = new IntersectionObserver((entries) => {
        messageLatestRef.current?.scrollIntoView({
          behavior: "smooth",
        });

        if (hasNextPage) setIsWaiting(true);
      ...
      if (user?.name !== data.name && !openChatBox) {
        customToast(
          ...
          <CustomNotifyComponent
            content={"A new messenger is comming !!!"}
            onChatClick={toggleDrawer(true)}
          />,
        );
      }
    });
    return () => {
      socket.off("messageToNotify");
    };
  }, [socket, openChatBox]);
  ...
  useEffect(() => {
    ...
    const myInterval = setInterval(() => {
      if (newMessageList.length > 0 && user?._id === owner) {
        saveChat({
          presentationId: room,
          newChats: newMessageList.map((mess) => JSON.stringify(mess)),
        }).then((res) => {
          setNewMessageList([]);
        });
      }
  ...
  return (
    <div>
      ...
      <Drawer anchor={"right"} open={openChatBox} onClose={toggleDrawer(false)}>
        <div className={styles.chat_window}>
          ...
          >
            {isWaiting && (
              <img
                src={getLinkWithPrefix("/images/spinner.svg")}
                alt="LOADING..."
              />
            )}
          </div>
          <ChatMessage
            ...
            user={user}
            lastMessRef={lastMessRef}
            messageLatestRef={messageLatestRef}
          />
