        ...
        try {
            if (localStorage?.getItem("access_token")) {
                ...
                if (res?.status === "OK") {
                    ...
                    const [groupListRes, presentationListRes] =
                        await Promise.all([
                            getGroupByIds([
                                ...userInfo.myGroupIds,
                                ...userInfo.joinedGroupIds,
                            ]),
                    ...
                    presentationListRes?.data?.forEach(
                        (presentation) =>
                            (presentationListMap[presentation?._id] =
                                presentation),
                    );

                    userInfo.myGroups =
                        userInfo.myGroupIds?.map(
                            (code) => groupListMap[code],
                        ) || [];

                    userInfo.joinedGroups =
                        userInfo.joinedGroupIds?.map(
                            (code) => groupListMap[code],
                        ) || [];
        ...
        socket.on("startPresent", async (data) => {
            if (
                ...
                toast(
                    <div>
                        <p>
                            Presentation {data?.presentationName} is presenting,
